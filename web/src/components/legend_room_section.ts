...
export class LegendRoomSection {
    ...
    constructor(leaflet_map: L.Map, polygonFactory: PolygonFactory, rooms: Room[]) {
        ...
        let unique_room_types = new Map<string, string>()
        rooms.forEach((room, index) => {
            let room_type = unique_room_types.get(room.type)

            if(room_type === undefined){
                toggleBackgroundColor += partialRoomColor+startLightness+'%),'
                startLightness += 10

                unique_room_types.set(room.type, "")
            }
        })
        toggleBackgroundColor = toggleBackgroundColor.substring(0,toggleBackgroundColor.length-1)
    ...
    public remove(){
        for (const layer of this.roomShapes.values()) {
            this.leaflet_map.removeLayer(layer)
        }
    }
